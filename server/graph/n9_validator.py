...
async def _check_forbidden_chars(latex: str) -> tuple[bool, str | None]:
    ...
    Only checks lines after \\begin{document} — the preamble is static
    template content that is never generated by AI and should not be
    validated (false positives on \\newcommand definitions that contain
    tabular environments with & column separators).
    """
    lines = latex.split("\n")

    # Find the first line of the document body; skip everything before it.
    doc_start = 0
    for doc_start, line in enumerate(lines):
        if r"\begin{document}" in line:
            doc_start += 1  # start checking from the line *after* \begin{document}
            break
    ...
    for i in range(doc_start, len(lines)):
        line_num = i + 1  # 1-indexed
        stripped = lines[i].split("%", 1)[0]  # remove LaTeX comments

        # Track tabular environments across lines (the preamble's
        # \newcommand definitions have tabular bodies that span multiple
        # lines; the document body may likewise have multi-line tabulars
        # inside \resumeSubheading and \resumeProjectHeading expansions).
        if re.search(r"\\begin\{(tabular\*?|tabularx|longtable)\}", stripped):
            in_tabular = True
        if re.search(r"\\end\{(tabular\*?|tabularx|longtable)\}", stripped):
            in_tabular = False
            continue  # \end{tabular*} lines won't contain bare &

        if not in_tabular and "&" in stripped:
            raw_amps = len(re.findall(r"(?<!\\)&", stripped))
            if raw_amps > 0:
                errors.append(f"line {line_num}: raw '&' outside tabular")
